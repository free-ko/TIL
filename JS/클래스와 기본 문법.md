...
- 리터럴을 사용해 만든 객체처럼 클래스도 `getter`나 `setter`, 계산된 프로퍼티(computed property)를 포함할 수 있습니다.
...
- 클래스 필드의 이런 기능은 브라우저 환경에서 메서드를 `이벤트 리스너`로 설정해야 할 때 특히 유용합니다.
...
class MyClass {
  ...
  // MyClass는 constructor의 코드를 본문으로 갖는 함수입니다.
  // MyClass에서 정의한 일반 메서드나 getter, setter는 MyClass.prototype에 쓰입니다.
