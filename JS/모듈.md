...
2. 외부 사이트같이 다른 오리진에서 모듈 스크립트를 불러오려면 `fetch`와 `Cross-Origin` 요청 챕터에서 설명한 바와 같이 `CORS` 헤더가 필요합니다.
   모듈이 저장되어있는 원격 서버가 `Access-Control-Allow-Origin: * `헤더를 제공해야만 외부 모듈을 불러올 수 있습니다. 참고로 `*` 대신 페치(`fetch`)를 허용할 도메인을 명시할 수도 있습니다.
