...
- `getter`와 `setter`를 ‘실제’ 프로퍼티 값을 감싸는 래퍼(`wrapper`)처럼 사용하면, 프로퍼티 값을 원하는 대로 통제할 수 있습니다.
- 아래 예시에선 `name`을 위한 `setter`를 만들어 `user`의 이름이 너무 짧아지는 걸 방지하고 있습니다. 실제 값은 별도의 프로퍼티 `\_name`에 저장됩니다.
...
- `user`의 이름은 `\_name`에 저장되고, 프로퍼티에 접근하는 것은 `getter`와 `setter`를 통해 이뤄집니다.
...
- 기존 코드들은 그대로 두도록 합시다.
- 대신 `age`를 위한 `getter`를 추가해서 문제를 해결해 봅시다.
